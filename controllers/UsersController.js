...
class UsersController {
  static async postNew(request, response) {
    ...
    return response
      .status(201)
      .send({ id: response.insertedId, email: userEmail });
  }

  static async getMe(request, response) {
    const access_token = request.header('X-Token') || null;
    if (!access_token) return response.status(401).send({ error: 'Unauthorized' });
